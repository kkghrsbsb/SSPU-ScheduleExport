...
// 4. 自动下载课程表的 HTML 预览文件（文件名固定为 schedule.html）。
// 5. 请将下载的 schedule.html 放入 src/original/ 中。
...
(function () {
  ...
  function getSemester(mode) {
    try {
      ...
      if (mode === "grad") {
        if (window.$ && $("#frmright")[0]) {
          return $(
            $($("#frmright")[0].contentDocument).find("#div-table div")[0],
          ).html();
        }
      }
    } catch (error) {
  ...
  function normalizeActivities(rawdata) {
    ...
    if (sourcePeriodCount) {
      var inferredDays = Math.round(activities.length / sourcePeriodCount);
  ...
  function buildPrettyHtml(rawdata, semester) {
    ...
    var dayCount = Math.min(schedule.dayCount, dayLabels.length);
    dayLabels = dayLabels.slice(0, dayCount);
    ...
    var bodyRows = schedule.grid
      .map(function (row, rowIndex) {
        var cells = row
          ...
          .map(function (cell) {
            ...
            var courses = cell
              .map(function (course) {
                ...
                var weekText = parseWeekRanges(course.vaildWeeks);
                var meta = [teacherName, roomName].filter(Boolean).join(" · ");
          ...
          .join("");
        var timeLabel = PERIODS[rowIndex] ? " " + PERIODS[rowIndex] : "";
        return (
          "<tr>" +
          '<th class="period">第' +
          (rowIndex + 1) +
